...
(() => {
  ...
  const translations = {
    ua: {
      ...
      footer: {
        ...
        col2_title:        'Дії',
        col3_title:        'Юридичне',
        col4_title:        'Лист новин',
        newsletter_intro: 'Раз на місяць — лише статистику впливу й можливість долучитися до виїздів.',
        newsletter_email_ph: 'Ваш email',
        newsletter_btn: 'Підписатися',
        newsletter_ok: 'Дякуємо! Перевірте пошту за мить.',
        newsletter_err: 'Перевірте email або напишіть на info@icanwomen.org.',
    ...
    en: {
      ...
      footer: {
        ...
        col3_title:        'Legal',
        col4_title:        'Updates',
        newsletter_intro: 'Once a month: impact tally + caravan openings. Zero growth-hack spam.',
        newsletter_email_ph: 'Your email',
        newsletter_btn: 'Subscribe',
        newsletter_ok: 'Thanks! Check inbox for confirmation.',
        newsletter_err: 'Double-check email or write info@icanwomen.org.',
  ...
  function setLanguage(lang) {
    ...
    const nf = document.getElementById('newsletter-feedback');
  ...
  async function submitNewsletter(e) {
    ...
    if (!validateForm('newsletter-form')) return;

    const endpoint = CONFIG.forms.newsletter;
    const langBundle = translations[currentLang]?.footer;

    if (!endpoint || /^https:\/\/formspree\.io\/f\/YOUR_/i.test(endpoint)) {
      feedback.textContent = langBundle?.newsletter_err || '';
      return;
    }

    const fd = new FormData(form);
    if (fd.get('email')) fd.set('_replyto', String(fd.get('email')).trim());
    fd.set('_subject', 'ICAN Newsletter');
    ...
    try {
      const res = await fetch(endpoint, { method: 'POST', body: fd, headers: { Accept: 'application/json' } });
      feedback.textContent = langBundle?.[res.ok ? 'newsletter_ok' : 'newsletter_err'] || '';
